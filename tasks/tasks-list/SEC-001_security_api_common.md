...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-004, REQ-FUNC-049, 6.2 api_key` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `API Key hash 검증, key status 확인, `last_used_at` 갱신 command 구현`의 보안 검증 흐름과 실패 오류 코드를 정의한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `DB-002, DB-008, PLAT-001`가 완료되어 있음
- When: `API Key hash 검증, key status 확인, `last_used_at` 갱신 command 구현` 작업을 수행함
- Then: SRS 관련 섹션 `REQ-FUNC-004, REQ-FUNC-049, 6.2 api_key`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `API Key hash 검증, key status 확인, `last_used_at` 갱신 command 구현` 작업이 실행됨
