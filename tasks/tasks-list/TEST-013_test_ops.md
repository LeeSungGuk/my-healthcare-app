...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-033~035, REQ-NF-006, REQ-NF-035` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `운영 로그 마스킹, threshold alert payload, review queue 수집 테스트 작성`의 Given/When/Then 시나리오를 테스트 파일로 분리한다.
...
- Given: `운영 로그 마스킹, threshold alert payload, review queue 수집 테스트 작성` 요구사항이 구현되지 않은 상태임
- When: 신규 테스트를 실행함
