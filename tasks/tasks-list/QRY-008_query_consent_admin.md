...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-036, REQ-FUNC-059, REQ-NF-023` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `elder/device 동의 상태, memory enabled, deletion job status 조회 구현` 조회 DTO, filter, sort, pagination 또는 export 옵션을 정의한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `DB-003, DB-008, CT-006`가 완료되어 있음
- When: `elder/device 동의 상태, memory enabled, deletion job status 조회 구현` 작업을 수행함
- Then: SRS 관련 섹션 `REQ-FUNC-036, REQ-FUNC-059, REQ-NF-023`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `elder/device 동의 상태, memory enabled, deletion job status 조회 구현` 작업이 실행됨
