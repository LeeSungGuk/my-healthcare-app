...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-049, REQ-FUNC-059, REQ-NF-032, REQ-NF-044, 6.7 COST-004~007` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] SRS 데이터 모델과 `API Key `last_used_at`, deletion job status, quota/budget config, AI run metadata 보강 schema 작성` 요구에 맞는 Prisma model 초안을 작성한다.
...
- Given: SRS 관련 섹션 `REQ-FUNC-049, REQ-FUNC-059, REQ-NF-032, REQ-NF-044, 6.7 COST-004~007`이 주어짐
- When: `DB-008` Prisma schema와 migration을 검토함
- Then: 필수 필드, key, index, relation이 누락 없이 정의되어야 한다.

Scenario 2: 선행 태스크와 연결됨
- Given: 선행 태스크 `DB-002, DB-006, CT-007, CT-008`가 완료됨
- When: `API Key `last_used_at`, deletion job status, quota/budget config, AI run metadata 보강 schema 작성` migration을 적용함
