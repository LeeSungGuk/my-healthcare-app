...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `6.2.1~6.2.5, REQ-FUNC-014, REQ-FUNC-022~026, REQ-NF-029~031` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] SRS 데이터 모델과 ``usage_event`, `poc_report_aggregate`, `audit_log` schema와 집계/query index 작성` 요구에 맞는 Prisma model 초안을 작성한다.
...
- Given: SRS 관련 섹션 `6.2.1~6.2.5, REQ-FUNC-014, REQ-FUNC-022~026, REQ-NF-029~031`이 주어짐
- When: `DB-006` Prisma schema와 migration을 검토함
- Then: 필수 필드, key, index, relation이 누락 없이 정의되어야 한다.

Scenario 2: 선행 태스크와 연결됨
- Given: 선행 태스크 `DB-002, DB-003`가 완료됨
- When: ``usage_event`, `poc_report_aggregate`, `audit_log` schema와 집계/query index 작성` migration을 적용함
