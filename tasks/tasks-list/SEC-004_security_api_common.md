...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `6.1 Standard Error Codes, REQ-FUNC-015, REQ-FUNC-045` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `표준 오류 응답 mapper 구현: INVALID_REQUEST, UNAUTHORIZED, FORBIDDEN, LLM_TIMEOUT, UPSTREAM_ERROR, SAFETY_FILTERED, INTERNAL_ERROR`의 보안 검증 흐름과 실패 오류 코드를 정의한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `CT-002, CT-003`가 완료되어 있음
- When: `표준 오류 응답 mapper 구현: INVALID_REQUEST, UNAUTHORIZED, FORBIDDEN, LLM_TIMEOUT, UPSTREAM_ERROR, SAFETY_FILTERED, INTERNAL_ERROR` 작업을 수행함
- Then: SRS 관련 섹션 `6.1 Standard Error Codes, REQ-FUNC-015, REQ-FUNC-045`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `표준 오류 응답 mapper 구현: INVALID_REQUEST, UNAUTHORIZED, FORBIDDEN, LLM_TIMEOUT, UPSTREAM_ERROR, SAFETY_FILTERED, INTERNAL_ERROR` 작업이 실행됨
