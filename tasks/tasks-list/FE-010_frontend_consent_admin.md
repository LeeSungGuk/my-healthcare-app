...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-059, REQ-NF-038` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `User & Consent Admin View 구현: elder/device, consent state, memory enabled, deletion job status` 화면 또는 component를 Tailwind CSS와 shadcn/ui 기준으로 구현한다.
...
- When: `User & Consent Admin View 구현: elder/device, consent state, memory enabled, deletion job status` 화면을 열람하거나 action을 실행함
