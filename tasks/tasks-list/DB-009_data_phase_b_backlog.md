...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `6.2.1~6.2.6, REQ-FUNC-040, REQ-FUNC-046~047` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] SRS 데이터 모델과 ``sensor_event` schema와 Phase B index를 migration backlog로 분리 관리` 요구에 맞는 Prisma model 초안을 작성한다.
...
- Given: SRS 관련 섹션 `6.2.1~6.2.6, REQ-FUNC-040, REQ-FUNC-046~047`이 주어짐
- When: `DB-009` Prisma schema와 migration을 검토함
- Then: 필수 필드, key, index, relation이 누락 없이 정의되어야 한다.

Scenario 2: 선행 태스크와 연결됨
- Given: 선행 태스크 `DB-001, DB-003`가 완료됨
- When: ``sensor_event` schema와 Phase B index를 migration backlog로 분리 관리` migration을 적용함
