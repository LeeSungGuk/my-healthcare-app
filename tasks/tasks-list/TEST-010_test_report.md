...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-022~026, REQ-NF-030~031, REQ-NF-041` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `PoC report JSON/CSV 필수 지표, active device distinct, 원문/PII 미포함 테스트 작성`의 Given/When/Then 시나리오를 테스트 파일로 분리한다.
...
- Given: `PoC report JSON/CSV 필수 지표, active device distinct, 원문/PII 미포함 테스트 작성` 요구사항이 구현되지 않은 상태임
- When: 신규 테스트를 실행함
