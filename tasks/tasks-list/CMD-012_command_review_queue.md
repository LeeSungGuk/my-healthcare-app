...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-035, REQ-NF-011~013, REQ-NF-035` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `위험/오류/timeout/신고 대상 이벤트를 review queue/evaluation_sample 후보로 생성하는 command 구현` command 입력 DTO와 validation rule을 구현한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `DB-005, CMD-007, CMD-008`가 완료되어 있음
- When: `위험/오류/timeout/신고 대상 이벤트를 review queue/evaluation_sample 후보로 생성하는 command 구현` 작업을 수행함
- Then: SRS 관련 섹션 `REQ-FUNC-035, REQ-NF-011~013, REQ-NF-035`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `위험/오류/timeout/신고 대상 이벤트를 review queue/evaluation_sample 후보로 생성하는 command 구현` 작업이 실행됨
