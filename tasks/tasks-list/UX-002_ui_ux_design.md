...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `6.4, REQ-FUNC-051~055` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `Web API Playground request builder/response viewer/snippet UX 상태 정의: loading, validation error, success, standard error`의 사용자, 목적, 화면 범위, 금지 표시 정보를 정의한다.
...
- Given: SRS 관련 섹션 `6.4, REQ-FUNC-051~055`이 주어짐
- When: `UX-002` UX 산출물을 검토함
- Then: 대상 사용자, 화면 범위, 표시 필드, 금지 표시 항목이 명확해야 한다.

Scenario 2: 구현자가 바로 사용할 수 있음
- Given: FE 태스크가 `Web API Playground request builder/response viewer/snippet UX 상태 정의: loading, validation error, success, standard error`를 구현하려고 함
- When: UX 문서를 참조함
