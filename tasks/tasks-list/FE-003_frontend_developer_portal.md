...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-002, REQ-FUNC-050, 6.4` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `API Docs UI 구현: Swagger 또는 API schema, enum, error code, cURL/TypeScript sample` 화면 또는 component를 Tailwind CSS와 shadcn/ui 기준으로 구현한다.
...
- When: `API Docs UI 구현: Swagger 또는 API schema, enum, error code, cURL/TypeScript sample` 화면을 열람하거나 action을 실행함
