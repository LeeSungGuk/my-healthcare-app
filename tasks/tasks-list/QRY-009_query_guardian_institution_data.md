...
- [ ] `보호자/기관용 요약 리포트 데이터 조회 계약 구현: 원문 미노출, 공유 동의/RBAC 적용` 조회 DTO, filter, sort, pagination 또는 export 옵션을 정의한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `DB-005, SEC-002, SEC-005, QRY-004`가 완료되어 있음
- When: `보호자/기관용 요약 리포트 데이터 조회 계약 구현: 원문 미노출, 공유 동의/RBAC 적용` 작업을 수행함
- Then: SRS 관련 섹션 `REQ-FUNC-027, REQ-FUNC-029~032, REQ-FUNC-039`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `보호자/기관용 요약 리포트 데이터 조회 계약 구현: 원문 미노출, 공유 동의/RBAC 적용` 작업이 실행됨
