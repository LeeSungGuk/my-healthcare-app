...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `API-008, REQ-FUNC-027, REQ-FUNC-030~032, REQ-FUNC-042` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] ``/api/v2/report/kpi` 주간 정서, 수면 분석, 기관 성과 JSON 구현 백로그 등록`를 Phase B/Post-MVP backlog로 등록하고 MVP blocking scope에서 제외한다.
...
- When: ``/api/v2/report/kpi` 주간 정서, 수면 분석, 기관 성과 JSON 구현 백로그 등록` backlog를 조회함
