...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-NF-007, REQ-FUNC-002, REQ-FUNC-050~055` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `첫 API 성공 30분 이내 onboarding 측정 스크립트와 시나리오 작성`의 측정 지표, threshold, 수집 위치를 정의한다.
...
Scenario 1: 정상 경로 수행
- Given: 선행 태스크 `FE-002~FE-007, TEST-014`가 완료되어 있음
- When: `첫 API 성공 30분 이내 onboarding 측정 스크립트와 시나리오 작성` 작업을 수행함
- Then: SRS 관련 섹션 `REQ-NF-007, REQ-FUNC-002, REQ-FUNC-050~055`의 기대 동작 또는 산출물이 충족되어야 한다.

Scenario 2: 실패 또는 제한 조건 처리
- Given: 필수 입력, 권한, tenant, consent, quota, schema 중 하나가 유효하지 않음
- When: `첫 API 성공 30분 이내 onboarding 측정 스크립트와 시나리오 작성` 작업이 실행됨
