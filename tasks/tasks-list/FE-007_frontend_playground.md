...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `REQ-FUNC-055` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] `현재 request payload 기반 cURL/TypeScript snippet 생성 UI 구현` 화면 또는 component를 Tailwind CSS와 shadcn/ui 기준으로 구현한다.
...
- When: `현재 request payload 기반 cURL/TypeScript snippet 생성 UI 구현` 화면을 열람하거나 action을 실행함
