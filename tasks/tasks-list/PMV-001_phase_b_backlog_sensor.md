...
- 목적: SRS `실버케어_SRS_v0.5_단일구조.md`의 `API-005, REQ-FUNC-040, REQ-FUNC-046~047` 요구를 구현 가능한 작업 단위로 완성한다.
...
- [ ] ``/api/v2/sensor/ingest` DTO, validation, tenant/device/elder 연결 구현 백로그 등록`를 Phase B/Post-MVP backlog로 등록하고 MVP blocking scope에서 제외한다.
...
- When: ``/api/v2/sensor/ingest` DTO, validation, tenant/device/elder 연결 구현 백로그 등록` backlog를 조회함
